...
public class CameraControl : MonoBehaviour {
	...
	private void FindAveragePosition()
	{
        ...
        {
            if (!m_Spawner.m_enemies[i].gameObject.activeSelf) // if the target is not active it will continue
                continue;
	...
	private float FindRequiredSize()
	{
        ...
        {
            if (!m_Spawner.m_enemies[i].gameObject.activeSelf)// if they aren't active countinue on to the next target
                continue;
            else
            {
                // otherwise, find the position of the target in the camera loacl space
